...
import data_manager
import push_results
from soc_engine import compute_soc_scores
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (SOC Engine) ===")
        ...
        for win in config.WINDOWS:
            min_bars = win + config.THRESH_STD_WIN + 5
        ...
        top_etfs    = [
            {"ticker": t, "soc_score": float(s), "best_window": int(w)}
        ...
        for win, score_dict in window_results.items():
            ...
            windows_tab2[str(win)] = {
                "top_etfs":    [{"ticker": t, "soc_score": float(s)} for t, s in sw[:config.TOP_N]],
    ...
    print(f"\n=== SOC Engine complete: {tab1_path.name}, {tab2_path.name} ===")
